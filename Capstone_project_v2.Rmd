...
# Remove observations with missing values in independent variable columns
income_pp_year_as_row <- filter(income_pp_year_as_row, !is.na(Income_per_person))

# Change the N/A to mean 
#income_pp_year_as_row$Income_per_person [which(is.na(income_pp_year_as_row$Income_per_person))] <- mean(income_pp_year_as_row$Income_per_person, na.rm = TRUE)
...
# Density Plot for income_per_person_as_row
...
# Create a text for income plots
...
# Empirical Cumulative Density Function Plot for income_per_person_as_row
...
# Empirical Cumulative Density Function for life_expectancy_year_as_row
...
# Empirical Cumulative Density Function for total_health_spending_pp_year_as_row
...
# Remove observations with missing values in independent variable column
...
# Empirical Cumulative Density Function for government_health_spending_pp_year_as_row
...
# Remove observations with missing values in independent variable and country columns
...
# Create a text for food supply plots
...
# Empirical Cumulative Density Function for food_supply_year_as_row
...
subset(food_supply_year_as_row, Food_Supply < 1500)
...
# Empirical Cumulative Density Function for improved_drinking_water_year_as_row
...
# Join all "year_as_row"" data frame using inner_join, not full_Join
...
# cor_income_life <- cor(income_vs_life_expectancy_in_1960n2010$Income_per_person, income_vs_life_expectancy_in_1960n2010$Life_Expectancy, use = "complete.obs", method="pearson")
...
# cor_life_tothealthspend <- cor(life_total_gov_spend_in_every5yrs$Total_Health_Spending_Per_Person, life_total_gov_spend_in_every5yrs$Life_Expectancy, use = "complete.obs")
...
# cor_life_govhealthspend <- cor(life_total_gov_spend_in_every5yrs$Government_Health_Spending_Per_Person, life_total_gov_spend_in_every5yrs$Life_Expectancy, use = "complete.obs")
...
#cor_life_water <- cor(life_water_in_2yrs$Improved_Water, life_water_in_2yrs$Life_Expectancy, use = "complete.obs")
...
# Create subsets for data from 1998 to 2001. 2002 data will be used. (for regression)
...
# Calculate the total of different cancer deaths in 2010 and create a new column for the total (for creating plots for total cancer deaths and regression)
...
# Create a text for cancer death plots
...
# Empirical Cumulative Density Function Plot for total_cancer_death_2002
...
mean(output_forest_wholeds$rsq) #0.78

# Create a regression model with 4 stronger predictor variables based on Best Subset and Stepwise Forward
...
# Split the dataset to a training set and a testing set with split ratio 0.5
set.seed(102)
split = sample.split(all_variables_last5yrs$Life_Expectancy, SplitRatio = 0.5)
Train1 = subset(all_variables_last5yrs, split == TRUE)
Test1 = subset(all_variables_last5yrs, split == FALSE)

# Create a regression model with all 6 predictor variables
mod_cv1_6p <- lm(Life_Expectancy ~ Income_per_person + Food_Supply + Improved_Water + All_Cancer_Death + Total_Health_Spending_Per_Person + Government_Health_Spending_Per_Person, data = Train1)
summary(mod_cv1_6p)

SSE=sum(mod_cv1_6p$residuals^2)
SSE

# 3-fold cross validation

cv.lm(Train1, form.lm = formula(Life_Expectancy ~ Income_per_person, Improved_Water, Food_Supply, Total_Health_Spending_Per_Person, Government_Health_Spending_Per_Person, All_Cancer_Death), m=3)


# Make a prediction on the testing set with 6 predictor variables

PredictTest_cv1_6p <- predict(mod_cv1_6p, newdata = Test1)

# Calculate Out-of-sample (test) R-Squared with 6 predictor variables

SSE_cv1_6p = sum((Test1$Life_Expectancy - PredictTest_cv1_6p) ^ 2)
SST_cv1_6p = sum((Test1$Life_Expectancy - mean(all_variables_last5yrs$Life_Expectancy))^ 2)
1 - (SSE_cv1_6p/SST_cv1_6p)

# Out-of-sample R-Squared = 0.639 

# Calculate RMSE for Test1

RMSE1 = sqrt(SSE_cv1_6p/nrow(Test1))

# Plot to choose the best threshold value

# ROCRperf1 = performance(PredictTest_cv1_6p, "True Positive Rate", "False Negative Rate")

# plot(ROCRperf1, colorize = TRUE, print.cutoffs.at=seq(0.1, 0.1), text.adj = c(-0.2, 1.7))

## Split the dataset to a training set and a testing set with split ratio 0.6
set.seed(103)
split2 = sample.split(all_variables_last5yrs$Life_Expectancy, SplitRatio = 0.6)
Train2 = subset(all_variables_last5yrs, split2 == TRUE)
Test2 = subset(all_variables_last5yrs, split2 == FALSE)

# Create a regression model with all 6 predictor variables
mod_cv2_6p <- lm(Life_Expectancy ~ Income_per_person + Food_Supply + Improved_Water + All_Cancer_Death + Total_Health_Spending_Per_Person + Government_Health_Spending_Per_Person, data = Train2)
summary(mod_cv2_6p)

SSE=sum(mod_cv2_6p$residuals^2)
SSE

# Make a prediction on the testing set with 6 predictor variables
PredictTest_cv2_6p <- predict(mod_cv2_6p, newdata = Test2)

# Calculate Out-of-sample (test) R-Squared with 6 predictor variables

SSE_cv2_6p = sum((Test1$Life_Expectancy - PredictTest_cv2_6p) ^ 2)
SST_cv2_6p = sum((Test1$Life_Expectancy - mean(all_variables_last5yrs$Life_Expectancy))^ 2)
1 - SSE_cv2_6p/SST_cv2_6p

# Out-of-sample R-Squared = -0.574

# Calculate RMSE for Test2

RMSE2 = sqrt(SSE_cv2_6p/nrow(Test2))


## Split the dataset to a training set and a testing set with split ratio 0.7
set.seed(104)
split3 = sample.split(all_variables_last5yrs$Life_Expectancy, SplitRatio = 0.7)
Train3 = subset(all_variables_last5yrs, split3 == TRUE)
Test3 = subset(all_variables_last5yrs, split3 == FALSE)

# Create a regression model with all 6 predictor variables
mod_cv3_6p <- lm(Life_Expectancy ~ Income_per_person + Food_Supply + Improved_Water + All_Cancer_Death + Total_Health_Spending_Per_Person + Government_Health_Spending_Per_Person, data = Train3)
summary(mod_cv3_6p)

SSE=sum(mod_cv3_6p$residuals^2)
SSE

# Make a prediction on the testing set with 6 predictor variables
PredictTest_cv3_6p <- predict(mod_cv3_6p, newdata = Test3)

# Calculate Out-of-sample (test) R-Squared with 6 predictor variables

SSE_cv3_6p = sum((Test1$Life_Expectancy - PredictTest_cv3_6p) ^ 2)
SST_cv3_6p = sum((Test1$Life_Expectancy - mean(all_variables_last5yrs$Life_Expectancy))^ 2)
1 - (SSE_cv3_6p/SST_cv3_6p)

# Out-of-sample R-Squared = -0.574

# Calculate RMSE for Test3

RMSE3 = sqrt(SSE_cv3_6p/nrow(Test3))
RMSE3

## Split the dataset to a training set and a testing set with split ratio 0.8
set.seed(105)
split4 = sample.split(all_variables_last5yrs$Life_Expectancy, SplitRatio = 0.8)
Train4 = subset(all_variables_last5yrs, split4 == TRUE)
Test4 = subset(all_variables_last5yrs, split4 == FALSE)

# Create a regression model with all 6 predictor variables
mod_cv4_6p <- lm(Life_Expectancy ~ Income_per_person + Food_Supply + Improved_Water + All_Cancer_Death + Total_Health_Spending_Per_Person + Government_Health_Spending_Per_Person, data = Train4)
summary(mod_cv4_6p)

SSE=sum(mod_cv4_6p$residuals^2)
SSE

# Make a prediction on the testing set with 6 predictor variables
PredictTest_cv4_6p <- predict(mod_cv4_6p, newdata = Test4)

# Calculate Out-of-sample (test) R-Squared with 6 predictor variables

SSE_cv4_6p = sum((Test1$Life_Expectancy - PredictTest_cv4_6p) ^ 2)
SST_cv4_6p = sum((Test1$Life_Expectancy - mean(all_variables_last5yrs$Life_Expectancy))^ 2)
1 - (SSE_cv4_6p/SST_cv4_6p)

# Out-of-sample R-Squared = -0.698

# Calculate RMSE for Test4

RMSE4 = sqrt(SSE_cv4_6p/nrow(Test4))
RMSE4

## Choose 0.7 threshold value (6 variables model with 0.7 threshold value has the highest Adj R-Squared)

# Create a regression model with 4 significant predictor variables
mod_cv3_4p <- lm(Life_Expectancy ~ Income_per_person + Food_Supply + Improved_Water + All_Cancer_Death, data = Train3)
summary(mod_cv3_4p)

SSE=sum(mod_cv3_4p$residuals^2)
SSE

# Make a prediction on the testing set with 4 significant predictor variables
PredictTest_cv3_4p <- predict(mod_cv3_4p, newdata = Test3)

# Calculate Out-of-sample (test) R-Squared with 4 significant variables

SSE_cv3_4p = sum((Test1$Life_Expectancy - PredictTest_cv3_4p) ^ 2)
SST_cv3_4p = sum((Test1$Life_Expectancy - mean(all_variables_last5yrs$Life_Expectancy))^ 2)
1 - (SSE_cv3_4p/SST_cv3_4p)

# Out-of-sample R-Squared = -1

# Calculate RMSE for Test4

RMSE3_4p = sqrt(SSE_cv3_4p/nrow(Test3))
RMSE3_4p

####################

## Use different methods to select predictor variables 
# Create Best Subsets Regression
# ols_best_subset(mod_cv1_6p)

# Create Stepwise Forward Regression 
# ols_step_forward(mod_cv1_6p)

# Create Random forest

#  output_forest_cv1 <- randomForest(Life_Expectancy ~ Income_per_person + Food_Supply + Improved_Water + All_Cancer_Death + Total_Health_Spending_Per_Person + Government_Health_Spending_Per_Person, data = Train1, importance=TRUE)

# View the Random forest results

# print(output_forest_cv1) 

# Importance of each predictor (Random forest)

# print(importance(output_forest_cv1)) 

# Create two more regression models based on previous testings and determine which model is the best one. 
# Create a regression model for a testing set with 4 predictor variables: Income, food, improved water source, and cancer death

# mod_cv1_4p_iwfc <- lm(Life_Expectancy ~ Income_per_person + Food_Supply + Improved_Water + All_Cancer_Death, data = Train1)
# summary(mod_cv1_4p_iwfc)

# SSE=sum(mod_cv1_4p_iwfc$residuals^2)
# SSE

# Make a prediction on the testing set with 4 predictor variables
# PredictTest1_4p_iwfc <- predict(mod_cv1_4p_iwfc, newdata = Test1)

# Calculate Out-of-sample (test) R-Squared with 4 predictor variables

# SSE_cv1_4p_iwfc = sum((Test1$Life_Expectancy - PredictTest1_4p_iwfc) ^ 2)
# SST_cv1_4p_iwfc = sum((Test1$Life_Expectancy - mean(all_variables_last5yrs$Life_Expectancy))^ 2)
# 1 - SSE_cv1_4p_iwfc/SST_cv1_4p_iwfc
...
# Create a regression model for a testing set with 5 predictor variables: Income, food, improved water source, total health spending, and government health spending

# AllVariables2002Log_5p <- lm(Life_Expectancy ~ Income_per_person + Food_Supply + Improved_Water + Total_Health_Spending_Per_Person + Government_Health_Spending_Per_Person, data = Train2002)
# summary(AllVariables2002Log_5p)

# SSE=sum(AllVariables2002Log_5p$residuals^2)
# SSE

# Make a prediction on the testing set with 4 predictor variables
# PredictTest2002_5p <- predict(AllVariables2002Log_5p, newdata = Test2002)

# Calculate Out-of-sample (test) R-Squared with 4 predictor variables

# SSE_5p = sum((Test2002$Life_Expectancy - PredictTest2002_5p) ^ 2)
# SST_5p = sum((Test2002$Life_Expectancy - mean(all_variables_2002$Life_Expectancy))^ 2)
# 1 - SSE_5p/SST_5p
