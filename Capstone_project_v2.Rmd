...
# Remove observations with missing values in independent variable column
food_supply_year_as_row <- filter(food_supply_year_as_row, !is.na(Food_Supply))
...
# Create a text for income plots
food_grob <- grobTree(textGrob(food_plot_label, x=0.5,  y=0.9, hjust=0,
  gp=gpar(col="black", fontsize=10, fontface="italic")))
...
food_grob2 <- grobTree(textGrob(food_plot_label, x=0.5,  y=0.15, hjust=0,
  gp=gpar(col="black", fontsize=10, fontface="italic")))
...
subset(food_supply_year_as_row, Food_Supply < 1000)
...
# Descriptive Statistics of total_cancer_death_2002
summary(total_cancer_death_2002)

# Check Outliers of total_cancer_death_2001
...
# Multiple Linear Regression using dataset all_year_as_row_2002 (latest years of cancer death data)

all_variables_2002 <- total_cancer_death_2002
...
# Split the dataset to a training set and a testing set
set.seed(100)
split = sample.split(all_variables_2002$Life_Expectancy, SplitRatio = 0.7)
Train2002 = subset(all_variables_2002, split == TRUE)
Test2002 = subset(all_variables_2002, split == FALSE)

# Create a regression model with all 6 predictor variables
AllVariables2002Log_6p <- lm(Life_Expectancy ~ Income_per_person + Food_Supply + Improved_Water + All_Cancer_Death + Total_Health_Spending_Per_Person + Government_Health_Spending_Per_Person, data = Train2002)
summary(AllVariables2002Log_6p)

SSE=sum(AllVariables2002Log_6p$residuals^2)
SSE

# Make a prediction on the testing set with 6 predictor variables
str(Test2002)
PredictTest2002_6p <- predict(AllVariables2002Log_6p, newdata = Test2002)

# Calculate Out-of-sample (test) R-Squared with 6 predictor variables

SSE_6p = sum((Test2002$Life_Expectancy - PredictTest2002_6p) ^ 2)
SST_6p = sum((Test2002$Life_Expectancy - mean(all_variables_2002$Life_Expectancy))^ 2)
1 - SSE_6p/SST_6p

# Use different methods to select predictor variables
# Create Best Subsets Regression
ols_best_subset(AllVariables2002Log_6p)

# Create Stepwise Forward Regression 
ols_step_forward(AllVariables2002Log_6p)

# Create Random forest

output_forest <- randomForest(Life_Expectancy ~ Income_per_person + Food_Supply + Improved_Water + All_Cancer_Death + Total_Health_Spending_Per_Person + Government_Health_Spending_Per_Person, data = Train2002, importance=TRUE)
...
print(importance(output_forest)) 
...
AllVariables2002Log_4p <- lm(Life_Expectancy ~ Income_per_person + Food_Supply + Improved_Water + All_Cancer_Death, data = Train2002)
summary(AllVariables2002Log_4p)

SSE=sum(AllVariables2002Log_4p$residuals^2)
SSE

# Make a prediction on the testing set with 4 predictor variables
PredictTest2002_4p <- predict(AllVariables2002Log_4p, newdata = Test2002)

# Calculate Out-of-sample (test) R-Squared with 4 predictor variables

SSE_4p = sum((Test2002$Life_Expectancy - PredictTest2002_4p) ^ 2)
SST_4p = sum((Test2002$Life_Expectancy - mean(all_variables_2002$Life_Expectancy))^ 2)
1 - SSE_4p/SST_4p
